...
- **프롬프트 팝업**(클립 팝업 오른쪽) — 자주 쓰는 문구 보관함. 직접 등록하거나 클립 카드의 🔖로 승격.
- **왼쪽 사이드바** — 고정 구간 앱을 화면 왼쪽 가장자리에 표시(설정에서 on/off). 기본은 **자동 숨김** — 마우스가 벗어나고 설정한 시간(기본 3초)이 지나면 화면 밖으로 미끄러져 사라지고, 화면 왼쪽 끝에 마우스를 대면 다시 나온다. 설정에서 자동 숨김을 끄면 상시 표시.
