...
### 클립보드 팝업
- 텍스트/이미지 최근 20개 — 기본은 **메모리에만 보관**(재시작 시 사라짐). 설정에서 끄면 디스크 저장·유지
...
- 삭제(✕)는 목록에서만 제거 (디스크 저장 모드에서 본문 파일은 폴더에 보존 — 정리는 수동)
...
| 메모리에만 보관 | 클립을 디스크에 저장 안 함 (**기본 on · 보안**). 끄면 디스크 저장·재시작 후 유지 |
| 단축키 | 도크 단축키 키 변경 |
...
> 기본값(**메모리에만 보관**)에서는 클립 데이터를 디스크에 쓰지 않아 `clips.json`·`media\` 가 생성되지 않는다. 설정에서 끄면 아래 위치에 저장된다.

| 경로 | 내용 |
|------|------|
| `config.json` | 앱 목록 · 단축키 · 각종 설정 (원자적 저장 + `config.bak` 자동 복구) |
| `clips.json` | 클립 히스토리 메타데이터 *(디스크 저장 모드일 때만)* |
| `media\text\` | 텍스트·경로 클립 본문(`.txt`) *(디스크 저장 모드일 때만)* |
| `media\image\` | 이미지 클립 본문(`.png`) *(디스크 저장 모드일 때만)* |
| `icons\` | 런처 앱 아이콘 PNG 캐시 |
| `app\` | 배포된 실행 파일(`FocusClip.exe`) |

## 개인정보·보안

- **기본 메모리 전용** — 클립(텍스트·이미지·기록)을 디스크에 저장하지 않고 메모리에만 둔다. 앱을 끄면 사라진다(설정에서 끄면 디스크 저장).
