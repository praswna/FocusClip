...
- **경로 팝업**(도크 아래) — 복사한 파일 경로·URL을 이름+축약 경로로 분리 표시. **클릭 = 열기**(폴더·파일은 탐색기, URL은 브라우저), 📋 = 직전 창에 붙여넣기.
- **고정 팝업**(각 팝업 오른쪽) — 카드의 📌로 고정한 항목을 **클립용·경로용 각각 별도 창**에서 관리한다(클릭 동작은 기본 팝업과 동일 — 클립은 붙여넣기, 경로는 열기). 한 줄짜리 압축 카드라 여러 개를 고정해도 자리를 적게 먹고, 고정한 항목은 기본 팝업 목록에서 빠져 최근 항목이 밀리지 않는다.
