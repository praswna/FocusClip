...
- 카드별: 수정(✎) · **프롬프트로 저장(🔖, 텍스트만)** · 고정핀(📌) · 저장 위치 열기(📂) · 삭제(✕)
...
클립보드 히스토리와 별개로, **자주 쓰는 프롬프트·정형 문구를 직접 등록해두는 보관함**. 프롬프트가 하나라도 있으면 CapsLock 시 **클립보드 팝업 오른쪽**에 함께 뜬다(화면 우측 끝이면 왼쪽으로 뒤집힘). 비어 있으면 다른 팝업처럼 표시되지 않는다.
- 첫 항목은 **클립보드 카드의 🔖(프롬프트로 저장)로 승격**하거나(편집창이 본문을 채운 채 열림), **트레이 우클릭 → 「프롬프트 추가」**로 만든다
...
- **시스템 트레이** 상주 — 우클릭 메뉴: 설정 / 프롬프트 추가 / 저장 폴더 열기 / 종료
...
클립 본문(텍스트·이미지)은 **`사진\Screenshots\`** 에, 설정·기록 등 사용자 파일은 그 안의 **`사진\Screenshots\FocusClip\`** 에 모인다. 구버전 `%LOCALAPPDATA%\FocusClip`·`%APPDATA%\FocusClip` 데이터는 첫 실행 시 자동 이전.

> 클립은 기본적으로 **메모리에만** 있다. **고정핀(📌)을 누른 항목만** 파일(`clip_*.txt`/`clip_*.png` + `clips.json`)로 저장돼 재시작 후에도 유지된다. 고정하지 않은 클립은 파일로 남지 않는다.

| 경로 | 내용 |
|------|------|
| `사진\Screenshots\clip_*.txt` | **고정한** 텍스트·경로 클립 본문 |
| `사진\Screenshots\clip_*.png` | **고정한** 이미지 클립 본문 — Win+Shift+S 스크린샷과 같은 폴더 |
| `사진\Screenshots\FocusClip\config.json` | 앱 목록 · 단축키 · 각종 설정 (원자적 저장 + `config.bak` 자동 복구) |
| `사진\Screenshots\FocusClip\prompts.json` | 프롬프트 보관함(제목+본문, 전량 영구 저장) |
| `사진\Screenshots\FocusClip\clips.json` | **고정한** 클립 히스토리 메타데이터 |
| `사진\Screenshots\FocusClip\icons\` | 런처 앱 아이콘 PNG 캐시 |
| `%LOCALAPPDATA%\FocusClip\app\` | 배포된 실행 파일(`FocusClip.exe`) |
