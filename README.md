...
**`CapsLock`** 을 누르면 마우스 옆(커서가 있는 모니터)에 **런처 도크**가 뜨고, 그 주변에 팝업들이 함께 열린다. 열린 채로 다시 누르면 팝업 탭이 **클립 → 경로 → 프롬프트** 순으로 넘어간다(끝없이 순환). 닫을 때는 `Esc`·바깥 클릭·다른 키 입력.
...
| 키 | 동작 |
|----|------|
| `CapsLock` | 도크·팝업 열기 / 열린 상태에서 다시 누르면 탭 순환(클립 → 경로 → 프롬프트) (설정에서 변경 가능) |
...
클립은 기본적으로 메모리에만 둔다. Windows가 `사진\Screenshots`에 자동 저장한 캡처는 같은 PNG 경로를 재사용하고, 다른 앱에서 복사한 이미지만 압축해 메모리에 보관한다. **고정핀(📌)을 누른 항목만 기록에 남아** 재시작 후에도 유지된다(프롬프트는 전량 영구 저장).
